    ...
    <v-card>
        <v-layout>
            <v-navigation-drawer v-if="!isCamposGeneralesView" v-model="drawer" :rail="rail" permanent
                @click="rail = false" color="indigo-darken-1" class="border-0">
                <div class="p-2 py-7 flex items-center justify-center gap-2">
                    <div>
                        ...
                        <p v-if="!rail" class="font-bold text-2xl text-center w-full">MediCita <span class="mdi mdi-hospital-box"></span> </p>
                        ...
                        <v-list-group v-else :value="itemNavegation.value" fluid>
                            <template v-slot:activator="{ props }">
                                <v-list-item v-bind="props" :prepend-icon="itemNavegation.icon"
                                    :title="itemNavegation.title" active-class="bg-indigo-lighten-1"></v-list-item>
                            </template>
                            <div
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            nombre_usuario.value = store.state.nombre_usuario
        ...
        const filteredItems = computed(() => {
            if (store.state.role === 'usuario') {
                return ItemsNavegation.value.filter(item => item.value === 'doctor' || item.value === 'paciente' || item.value === 'internamiento' || item.value === 'visitas' || item.value==='altas' );
